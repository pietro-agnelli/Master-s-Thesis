...
# Definisco una distanza "buffer" per il time scaling.
h_buffer = 0.5   # [m] (Ora non usato per il controllo, solo per il logging)
...
def main():
    ...
    try:
        ...
        while t < 150.0:
            ...
            constrain_acc_matrix = np.eye(model.nq)
            constrain_acc_vector = np.ones(model.nq)*a_j
        ...
        if not log_time:
            ...
            ax_h.set_xlabel('t [s]')
            ax_h.set_ylabel('Valore CBF h (margine in m)')
